...
public class JwtService
{
    public static string GenerateToken(IEnumerable<Claim> claims, JwtOptions options)
    {
        ...
        var tokenOptions = new JwtSecurityToken(
            ...
            claims: claims,                 // Những thông tin mà bạn muốn lưu trữ trong token, thường là thông tin về người dùng như ID, email, vai trò, v.v. nằm trong payload
            expires: DateTime.Now.AddMinutes(options.ExpirationMinutes), // Token sẽ hết hạn sau bao lâu
            signingCredentials: signinCredentials
        );

        // Sau đó gọi JwtSecurityTokenHandler để tạo ra token dưới dạng chuỗi (string) từ các thông tin đã cung cấp ở trên
